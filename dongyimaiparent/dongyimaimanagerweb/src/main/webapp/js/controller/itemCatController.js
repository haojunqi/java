...
app.controller('itemCatController' ,function($scope,$controller   ,typeTemplateService,itemCatService){
	...
	$scope.storeRedis=function (){
		itemCatService.storeRedis().success(
			function (response){
				if (response.success){
	...
	$scope.save=function(){				
		...
		serviceObject.success(
			function(response){
				if(response.success){
					//重新查询
		        	/*itemCatService.findByParentId(1,10,$scope.parentId)*/
	...
	$scope.search=function(page,rows){
		itemCatService.findByParentId(page,rows,$scope.parentId).success(
 			function(response){
			$scope.list=response.rows;
 				$scope.paginationConf.totalItems=response.total;//更新总记录数
			}
...
/*		$scope.findByParentId=function (parentId){
		...
		itemCatService.findByParentId(page,rows,parentId).success(
			function (response){
